...
then
    ...
    # Exit with a success code while the aspell project is not expected to exist
    # on oss-fuzz.
    exit 0
fi
...
sed -i "s@https://github.com/GNUAspell/aspell-fuzz.git@-b ${BRANCH} https://github.com/${REPO}.git@" /tmp/ossfuzz/projects/${PROJECT_NAME}/Dockerfile
